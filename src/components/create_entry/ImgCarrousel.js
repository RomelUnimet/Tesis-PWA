...
export const ImgCarrousel = ({ entryImgState, setEntryImgState, setImgInputIsEmpty, fullscreen, setfullscreen }) => {
    ...
    const bindCarrousel = useDrag(
        ({ last, vxvy: [, vy], movement: [, my]}) => {
            ...
            if (last) {           //Hay que hacer una funcion que haga el close
                my > 50 ?  closeFS() : openFS()
              } else{
                  if(my===0){
                    api.start({ y:0, immediate: true })
                  }else{
                    if(my<65){
                        api.start({ y:0, height:window.innerHeight-(my*7.7), marginTop:`${my/12}`, backgroundColor:`rgb(0, 0, 0, ${(1-(my/100))})`, config:config.stiff })
                  }
                }
              } }
             
            },
        { initial: () => [0, y.get()], filterTaps: true, bounds: { top: 0 }, rubberband: false, axis:'y' }
        )

    const openFS = () => {
        api.start({y:0, height:window.innerHeight+5, backgroundColor:`rgb(0, 0, 0, ${1})`,marginTop:'0rem' ,config:config.gentle} )
