...
export const ImgEditor = ({ editorState, setEditorState }) => {
    ...
    const dummyImg = 'https://img.freepik.com/vector-gratis/paisaje-montanas-al-atardecer_52683-24164.jpg?size=626&ext=jpg'

    return (
        <div 
             //Slide Out del Cropper no funciona
             className={ true? 'animate__animated animate__fadeIn cropper-container' : 'animate__animated animate__slideOutDown cropper-container-exit' }
        >

           { true? //Cambiar Cuando se ponga a funcionar
           
            <Cropper
                style={{ height: '60%', width: "100%" }}
                initialAspectRatio={7 / 10}
                zoomTo={0.3}
                viewMode={3}
                background={false}
                responsive={true}
